...
```json
// packages/components/button/dist/manifest.json
{
  "name": "@ubits/button",
  "version": "1.0.0",
  "type": "component",
...
```typescript
// packages/components/button/src/ButtonAddon.ts
...
```typescript
// packages/components/button/src/ButtonComponent.ts
import { renderButton } from './ButtonProvider';
...
```css
/* packages/components/button/src/styles/button.css */
ubits-button {
...
- [ ] Crear estructura de carpetas en `packages/components/[nombre]`
...
- [ ] Exportar en `packages/components/index.ts`
